...
from local_tabnet import sparsemax
...
class TabNet(object):
  ...
  def encoder(self, data, reuse, is_training):
    ...
    with tf.compat.v1.variable_scope("Encoder", reuse=reuse):
      ...
      features = tf.compat.v1.layers.batch_normalization(
      ...
      for ni in range(self.num_decision_steps):
        ...
        transform_f1 = tf.compat.v1.layers.dense(
            masked_features,
            self.feature_dim * 2,
            name="Transform_f1",
            reuse=reuse_flag,
            use_bias=False)
        transform_f1 = tf.compat.v1.layers.batch_normalization(
            transform_f1,
        ...
        transform_f2 = tf.compat.v1.layers.dense(
            transform_f1,
            self.feature_dim * 2,
            name="Transform_f2",
            reuse=reuse_flag,
            use_bias=False)
        transform_f2 = tf.compat.v1.layers.batch_normalization(
            transform_f2,
            training=is_training,
            momentum=self.batch_momentum,
            virtual_batch_size=v_b)
        ...
        transform_f3 = tf.compat.v1.layers.dense(
            transform_f2,
            self.feature_dim * 2,
            name="Transform_f3" + str(ni),
            use_bias=False)
        transform_f3 = tf.compat.v1.layers.batch_normalization(
            transform_f3,
            training=is_training,
            momentum=self.batch_momentum,
            virtual_batch_size=v_b)
        ...
        transform_f4 = tf.compat.v1.layers.dense(
            transform_f3,
            self.feature_dim * 2,
            name="Transform_f4" + str(ni),
            use_bias=False)
        transform_f4 = tf.compat.v1.layers.batch_normalization(
            transform_f4,
            training=is_training,
            momentum=self.batch_momentum,
            virtual_batch_size=v_b)
        ...
        if ni > 0:
          ...
          scale_agg = tf.reduce_sum(
              input_tensor=decision_out, axis=1, keepdims=True) / (
        ...
        if ni < self.num_decision_steps - 1:
          ...
          mask_values = tf.compat.v1.layers.dense(
              features_for_coef,
              self.num_features,
              name="Transform_coef" + str(ni),
              use_bias=False)
          mask_values = tf.compat.v1.layers.batch_normalization(
              mask_values,
              training=is_training,
              momentum=self.batch_momentum,
              virtual_batch_size=v_b)
          ...
          total_entropy += tf.reduce_mean(
              input_tensor=tf.reduce_sum(
                  input_tensor=-mask_values * tf.math.log(mask_values + self.epsilon),
                  axis=1)) / (
                      self.num_decision_steps - 1)
  ...
  def classify(self, activations, reuse):
    ...
    with tf.compat.v1.variable_scope("Classify", reuse=reuse):
      logits = tf.compat.v1.layers.dense(activations, self.num_classes, use_bias=False)
  ...
  def regress(self, activations, reuse):
    ...
    with tf.compat.v1.variable_scope("Regress", reuse=reuse):
      predictions = tf.compat.v1.layers.dense(activations, 1)
      return predictions
